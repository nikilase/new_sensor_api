...
from app.api.models.influx import get_max_value, get_running_avg, write_line
from app.conf import config
from app.conf.config import influxdb, sensors
from app.helpers.my_logger import log_error, log_info, log_warn
...
def str_to_float_validate(
    value_str: str, min_val: float, max_val: float, value_type: str = "value"
) -> float | None:
    try:
        value_float = float(value_str)
    except Exception as e:
    ...
    if value_float < min_val or value_float > max_val or math.isnan(value_float):
        log_warn(
            "POST /Send",
            f"Inconsistent {value_type} reading of {value_float}! Discarding value!",
        )
        return None
    return value_float
...
def extract_and_send_sensor_data(sensor_data_json: dict):
    ...
    for k, v in sensor_data_json.items():
        match k:
            ...
            case _:
                log_warn(
                    "POST /Send",
                    f"Found new element in json with key {k} and value {v}",
                )
    ...
    if not sensor_data_values:
        log_error(
            "POST /Send",
            f"Received no valid sensor data value array!\n"
    ...
    for elem in sensor_data_values:
        ...
        match typ:
            ...
            case "miflora_temperature":
                miflora_temperature = str_to_float_validate(
                    val, -50, 50, "miflora_temperature"
                )
            ...
            case _:
                log_warn(
                    "POST /Send extract_and_send_sensor_data()",
...
def extract_and_send_stat_data(sensor_stat_json: dict):
    ...
    if sensor_type == "restart":
        ...
        log_error(
            "POST /post_stat",
            f"Received no valid message type!\n" f"Instead received {sensor_type}",
        )
        return 2
