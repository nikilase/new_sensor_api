...
from app.openweather.models.sqlite import get_from_db, get_from_sqlite, OWWeather
...
def get_from_openweather_api(lat: Decimal, long: Decimal):
    ...
    if last_update is None or datetime.now(
        tz=timezone.utc
    ) - last_update.last_updated_utc > timedelta(minutes=5):
        # Make request
        ...
        weather = OWWeather(
            lat=lat, long=long, json_data=ow_data, last_updated_utc=upd_time
        )
